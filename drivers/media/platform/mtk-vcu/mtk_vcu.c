...
#define time_check_end(timeout_ms, debug)                                      \
 do {                                                                          \
  time_ms_e = jiffies_to_msecs(jiffies);                                       \
  if ((time_ms_e - time_ms_s) > timeout_ms || debug)                           \
   pr_info("[VCU][Info] %s L:%d take %u timeout %u ms", __func__, __LINE__,    \
           time_ms_e - time_ms_s, timeout_ms);                                 \
 } while (0)
...
int vcu_ipi_send(struct platform_device *pdev, enum ipi_id id, void *buf,
 ...
 if (id <= IPI_VCU_INIT || id >= IPI_MAX || len > sizeof(send_obj.share_buf) ||
     buf == NULL) {
 ...
 if (vcu_ptr->abort || ret == 0) {
  ...
  dev_err(&pdev->dev, "vcu ipi %d ack wait interrupted by a signal", id);
  ret = -ERESTARTSYS;
  mutex_unlock(&vcu->vcu_mutex[i]);
  goto end;
 } else {
...
static int vcu_ipi_get(struct mtk_vcu *vcu, unsigned long arg) {
 ...
 ret = wait_event_freezable(vcu->get_wq[i], atomic_read(&vcu->ipi_got[i]));
...
static void vcu_gce_clear_inst_id(void *ctx) {
 ...
 for (i = 0; i < VCODEC_INST_MAX; i++) {
  if (vcu_ptr->gce_info[i].v4l2_ctx == ctx) {
   ...
   mutex_unlock(&vcu_ptr->vcu_share);
   if (temp > 0)
    vcu_aee_print("%s %p hndl %llu free id %d cnt %d %d\n", __func__, ctx,
                  gce_handle, i, temp, temp2);
   else if (temp2 > 0)
    pr_info("%s %p hndl %llu free id %d cnt %d %d\n", __func__, ctx, gce_handle,
            i, temp, temp2);
   else
    vcu_dbg_log("%s %p hndl %llu free id %d cnt %d %d\n", __func__, ctx,
                gce_handle, i, temp, temp2);
   return;
  }
 }
...
static void *vcu_check_gce_pa_base(struct mtk_vcu_queue *vcu_queue, u64 addr,
                                   u64 length) {
 ...
 list_for_each_safe(p, q, &vcu_queue->pa_pages.list) {
  ...
  if (addr >= (u64)tmp->pa && addr + length <= (u64)tmp->pa + PAGE_SIZE)
   return tmp;
 }
...
static void vcu_set_gce_cmd(struct cmdq_pkt *pkt, struct mtk_vcu *vcu,
 ...
 case CMD_SEC_WRITE:
  ...
  if (vcu_check_reg_base(vcu, addr, 4) == 0) {
#if defined(CONFIG_MTK_SVP_ON_MTEE_SUPPORT)
   cmdq_sec_pkt_write_reg(pkt, addr, data, CMDQ_IWC_H_2_MVA, dma_offset,
                          dma_size, 0, SEC_ID_WFD);
#endif
  } else {
...
static void vcu_gce_flush_callback(struct cmdq_cb_data data) {
 ...
 if (atomic_dec_and_test(&vcu->gce_job_cnt[i][core_id]) && j >= 0 &&
     vcu->gce_info[j].v4l2_ctx != NULL) {
  if (i == VCU_VENC) {
   venc_encode_unprepare(vcu->gce_info[j].v4l2_ctx, buff->cmdq_buff.core_id,
                         &vcu->flags[i]);
#if defined(CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT)
...
#endif
#endif
   venc_unlock(vcu->gce_info[j].v4l2_ctx, buff->cmdq_buff.core_id);
  }
 }
...
static void vcu_gce_timeout_callback(struct cmdq_cb_data data) {
 ...
 list_for_each_safe(p, q, &vcu_queue->pa_pages.list) {
  ...
  pr_info("%s: vcu_pa_pages %lx kva %lx data %lx\n", __func__, tmp->pa,
          tmp->kva, *(unsigned long *)tmp->kva);
 }
}

static int vcu_gce_cmd_flush(struct mtk_vcu *vcu, struct mtk_vcu_queue *q,
                             unsigned long arg) {
 int i, j, ret;
 unsigned char *user_data_addr = NULL;
 struct gce_callback_data buff;
 ...
 if (buff.cmdq_buff.cmds_user_ptr > 0) {
  user_data_addr = (unsigned char *)(unsigned long)buff.cmdq_buff.cmds_user_ptr;
 ...
 if (buff.cmdq_buff.codec_type >= VCU_CODEC_MAX ||
     core_id >= vcu->gce_th_num[buff.cmdq_buff.codec_type]) {
  pr_info("[VCU] %s invalid core(th) id %d\n", __func__, core_id);
  return -EINVAL;
 }

 cl = (buff.cmdq_buff.codec_type == VCU_VDEC) ? vcu->clt_vdec[core_id]
 ...
 if (j < 0)
  j = vcu_gce_set_inst_id(vcu->curr_ctx[i], buff.cmdq_buff.gce_handle);
 if (j < 0) {
  return -EINVAL;
 }
 ...
 if (atomic_read(&vcu->gce_job_cnt[i][core_id]) == 0 &&
     vcu->gce_info[j].v4l2_ctx != NULL) {
  if (i == VCU_VENC) {
   venc_encode_prepare(vcu->gce_info[j].v4l2_ctx, core_id, &vcu->flags[i]);
  }
 }
 vcu_dbg_log("vcu gce_info[%d].v4l2_ctx %p\n", j,
             (j >= 0) ? vcu->gce_info[j].v4l2_ctx : NULL);
 if (i == VCU_VENC) {
 ...
 if (buff.cmdq_buff.codec_type == VCU_VENC) {
  if (buff.cmdq_buff.secure != 0) {
   const u64 dapc_engine =
       (1LL << CMDQ_SEC_VENC_BSDMA) | (1LL << CMDQ_SEC_VENC_CUR_LUMA) |
       (1LL << CMDQ_SEC_VENC_CUR_CHROMA) | (1LL << CMDQ_SEC_VENC_REF_LUMA) |
       (1LL << CMDQ_SEC_VENC_REF_CHROMA) | (1LL << CMDQ_SEC_VENC_REC) |
       (1LL << CMDQ_SEC_VENC_SV_COMV) | (1LL << CMDQ_SEC_VENC_RD_COMV);
   ...
   pr_debug("[VCU] dapc_engine: 0x%llx, port_sec_engine: 0x%llx\n", dapc_engine,
            port_sec_engine);
#if defined(CONFIG_MTK_CMDQ_MBOX_EXT)
   cmdq_sec_pkt_set_data(pkt_ptr, dapc_engine, port_sec_engine,
                         CMDQ_SEC_KERNEL_CONFIG_GENERAL, CMDQ_METAEX_VENC);

   if (buff.cmdq_buff.secure == 3) {
#if defined(CONFIG_MTK_SVP_ON_MTEE_SUPPORT)
    // CMDQ MTEE hint
    pr_debug("[VCU] Use MTEE\n");
    cmdq_sec_pkt_set_mtee(pkt_ptr, (buff.cmdq_buff.secure == 3), SEC_ID_WFD);
#endif
   }
#endif
  }
 ...
 for (i = 0; i < cmds->cmd_cnt; i++) {
  vcu_set_gce_cmd(pkt_ptr, vcu, q, cmds->cmd[i], cmds->addr[i], cmds->data[i],
                  cmds->mask[i], vcu->gce_gpr[core_id], cmds->dma_offset[i],
                  cmds->dma_size[i]);
 }
 ...
 pr_info(
     "[VCU][%d] %s: buff %p type %d cnt %d order %d pkt %p hndl %llx %d %d\n",
     core_id, __func__, &vcu_ptr->gce_info[j].buff[i],
     buff.cmdq_buff.codec_type, cmds->cmd_cnt, buff.cmdq_buff.flush_order,
     pkt_ptr, buff.cmdq_buff.gce_handle, ret, j);

 /* flush cmd async */
 ret = cmdq_pkt_flush_threaded(pkt_ptr, vcu_gce_flush_callback,
                               (void *)&vcu_ptr->gce_info[j].buff[i]);
 if (ret < 0)
...
int vcu_set_codec_ctx(struct platform_device *pdev, void *codec_ctx,
                      struct vb2_buffer *src_vb, struct vb2_buffer *dst_vb,
                      unsigned long type) {
 struct mtk_vcu *vcu = platform_get_drvdata(pdev);
 pr_debug("[VCU] type %lu vcu_set_codec_ctx %p src_vb %p dst_vb %p\n", type,
          codec_ctx, src_vb, dst_vb);
...
int vcu_clear_codec_ctx(struct platform_device *pdev, void *codec_ctx,
                        unsigned long type) {
 struct mtk_vcu *vcu = platform_get_drvdata(pdev);
...
void *vcu_mapping_dm_addr(struct platform_device *pdev,
 ...
 if (dtcm_dmem_addr == 0UL || d_off > VCU_DMEM0_LEN(vcu)) {
  dev_info(&pdev->dev, "[VCU] %s: Invalid vma 0x%lx len %lx\n", __func__,
           dtcm_dmem_addr, VCU_DMEM0_LEN(vcu));
  return NULL;
 }
...
int vcu_compare_version(struct platform_device *pdev,
 ...
 cur_ver_num = sscanf(cur_version, "%d.%d.%d-rc%d", &cur_major, &cur_minor,
                      &cur_build, &cur_rel);
 if (cur_ver_num < 3)
  return -1;
 ver_num =
     sscanf(expected_version, "%d.%d.%d-rc%d", &major, &minor, &build, &rel);
 if (ver_num < 3)
  return -1;
...
void vcu_get_file_lock(void) { mutex_lock(&vpud_file_mutex); }
EXPORT_SYMBOL_GPL(vcu_get_file_lock);

void vcu_put_file_lock(void) { mutex_unlock(&vpud_file_mutex); }
...
static int vcu_ipi_handler(struct mtk_vcu *vcu, struct share_obj *rcv_obj) {
 ...
 if (rcv_obj->id < (int)IPI_MAX && ipi_desc[rcv_obj->id].handler != NULL) {
  non_ack = ipi_desc[rcv_obj->id].handler(rcv_obj->share_buf, rcv_obj->len,
...
static int vcu_init_ipi_handler(void *data, unsigned int len, void *priv) {
 ...
 if (vcu->run.signaled == 1u && run->signaled == 0u) {
  ...
  while (atomic_read(&vcu_ptr->gce_job_cnt[VCU_VDEC][0]) > 0 ||
  ...
  for (i = 0; i < VCU_CODEC_MAX; i++) {
   atomic_set(&vcu->ipi_got[i], 1);
   atomic_set(&vcu->ipi_done[i], 0);
   memset(&vcu->user_obj[i], 0, sizeof(struct share_obj));
   wake_up(&vcu->get_wq[i]);
   wake_up(&vcu->ack_wq[i]);
  }
...
static int mtk_vcu_open(struct inode *inode, struct file *file) {
 ...
 if (strcmp(current->comm, "camd") == 0)
  ...
  vcu_get_file_lock();
  if (vcud_task && (current->tgid != vcud_task->tgid ||
 ...
 pr_info("[VCU] %s name: %s pid %d tgid %d open_cnt %d current %p group_leader "
         "%p\n",
...
static int mtk_vcu_mmap(struct file *file, struct vm_area_struct *vma) {
 ...
 if (pa_start >= MAP_SHMEM_ALLOC_BASE && pa_end <= MAP_SHMEM_ALLOC_END) {
  vma->vm_pgoff = (unsigned long)(VCU_DMEM0_PHY(vcu_dev) >> PAGE_SHIFT);
  goto valid_map;
 }
 ...
 if (pa_start_base >= MAP_SHMEM_MM_BASE && pa_start_base < MAP_SHMEM_PA_BASE) {
  ...
  mem_buff_data.iova =
      (vcu_ptr->iommu_padding) ? (pa_start | 0x100000000UL) : pa_start;
  ...
  ret = mtk_vcu_set_buffer(vcu_queue, &mem_buff_data, src_vb, dst_vb);
  ...
  while (length > 0) {
   ...
   if (remap_pfn_range(vma, start, vma->vm_pgoff, PAGE_SIZE,
...
static long mtk_vcu_unlocked_ioctl(struct file *file, unsigned int cmd,
 ...
 case VCU_PA_ALLOCATION:
  ...
  vcu_dbg_log("[VCU] ALLOCATION %d va %llx, pa %llx, iova %llx\n",
              cmd == VCU_MVA_ALLOCATION, mem_buff_data.va, mem_buff_data.pa,
              mem_buff_data.iova);
 ...
 case VCU_PA_FREE:
  ...
  ret = (long)copy_from_user(&mem_buff_data, user_data_addr,
                             (unsigned long)sizeof(struct mem_obj));
  if ((ret != 0L) || (mem_buff_data.iova == 0UL && mem_buff_data.va == 0UL)) {
  ...
  if (ret != 0L) {
   pr_info("[VCU] VCU_FREE failed %d va %llx, pa %llx, iova %llx\n",
           cmd == VCU_MVA_FREE, mem_buff_data.va, mem_buff_data.pa,
           mem_buff_data.iova);
   return -EINVAL;
  }
 ...
 case VCU_CACHE_INVALIDATE_BUFF:
  ...
  if (vcu_ptr->iommu_padding)
   mem_buff_data.iova |= 0x100000000UL;
  ret = vcu_buffer_cache_sync(
      dev, vcu_queue, (dma_addr_t)mem_buff_data.iova, (size_t)mem_buff_data.len,
      (cmd == VCU_CACHE_FLUSH_BUFF) ? DMA_TO_DEVICE : DMA_FROM_DEVICE);
  if (ret < 0)
   return -EINVAL;
...
static long mtk_vcu_unlocked_compat_ioctl(struct file *file, unsigned int cmd,
 ...
 case VCU_GCE_WAIT_CALLBACK:
  ...
  ret = file->f_op->unlocked_ioctl(file, cmd, (unsigned long)share_data32);
  break;
 ...
 case COMPAT_VCU_PA_ALLOCATION:
  ...
  if (cmd == COMPAT_VCU_MVA_ALLOCATION)
   ret = file->f_op->unlocked_ioctl(file, (uint32_t)VCU_MVA_ALLOCATION,
                                    (unsigned long)data);
  else
   ret = file->f_op->unlocked_ioctl(file, (uint32_t)VCU_PA_ALLOCATION,
                                    (unsigned long)data);
 ...
 case COMPAT_VCU_PA_FREE:
  ...
  if (cmd == COMPAT_VCU_MVA_FREE)
   ret = file->f_op->unlocked_ioctl(file, (uint32_t)VCU_MVA_FREE,
                                    (unsigned long)data);
  else
   ret = file->f_op->unlocked_ioctl(file, (uint32_t)VCU_PA_FREE,
                                    (unsigned long)data);
  err = compat_put_vpud_allocation_data(data32, data);
  if (err != 0)
   return err;
 ...
 case COMPAT_VCU_CACHE_FLUSH_ALL:
  ret = file->f_op->unlocked_ioctl(file, (uint32_t)VCU_CACHE_FLUSH_ALL, 0);
  break;
 ...
 case COMPAT_VCU_CACHE_INVALIDATE_BUFF:
  ...
  if (cmd == COMPAT_VCU_CACHE_FLUSH_BUFF) {
   ret = file->f_op->unlocked_ioctl(file, (uint32_t)VCU_CACHE_FLUSH_BUFF,
                                    (unsigned long)data);
  } else {
   ret = file->f_op->unlocked_ioctl(file, (uint32_t)VCU_CACHE_INVALIDATE_BUFF,
                                    (unsigned long)data);
  }
...
int mtk_vcu_write(const char *val, const struct kernel_param *kp) {
 if (vcu_ptr != NULL && vcu_ptr->vdec_log_info != NULL && val != NULL) {
  memcpy(vcu_ptr->vdec_log_info->log_info, val,
         strnlen(val, LOG_INFO_SIZE - 1) + 1);
 } else {
 ...
 pr_info("[log wakeup VPUD] log_info %p vcu_ptr %p val %p: %s %lu\n",
         (char *)vcu_ptr->vdec_log_info->log_info, vcu_ptr, val, val,
...
int vcu_set_log(const char *val) { return mtk_vcu_write(val, NULL); }
...
static int mtk_vcu_probe(struct platform_device *pdev) {
 ...
 if (vcu_mtkdev[vcuid]->io_domain == NULL) {
  dev_err(dev, "[VCU] vcuid: %d get io_domain fail !!\n", vcuid);
  return -EPROBE_DEFER;
 }
 ...
 if (vcuid == 2)
  ...
  vcu_mtkdev[vcuid]->vdec_log_info =
      devm_kzalloc(dev, sizeof(struct log_test_nofuse), GFP_KERNEL);
  pr_info("[VCU] vdec_log_info %p %d vcuid %d vcu_ptr %p\n",
          vcu_mtkdev[vcuid]->vdec_log_info, (int)sizeof(struct log_test_nofuse),
          (int)vcuid, vcu_ptr);
 ...
 ret = vcu_ipi_register(pdev, IPI_VCU_INIT, vcu_init_ipi_handler, "vcu_init",
                        vcu);
 if (ret != 0) {
 ...
 ret = alloc_chrdev_region(&vcu_mtkdev[vcuid]->vcu_devno, 0, 1,
                           vcu_mtkdev[vcuid]->vcuname);
 if (ret < 0) {
  dev_err(dev, "[VCU] alloc_chrdev_region failed (%d)\n", ret);
  goto err_alloc;
 }
 ...
 vcu_mtkdev[vcuid]->vcu_device = device_create(
     vcu_mtkdev[vcuid]->vcu_class, NULL, vcu_mtkdev[vcuid]->vcu_devno, NULL,
     vcu_mtkdev[vcuid]->vcuname);
 ...
 for (i = 0; i < ARRAY_SIZE(vcu->gce_gpr); i++) {
  ret =
      of_property_read_u32_index(dev->of_node, "gce-gpr", i, &vcu->gce_gpr[i]);
  if (ret < 0)
   break;
 }
 ...
 vcu->clt_venc_sec[0] = cmdq_mbox_create(dev, vcu->gce_th_num[VCU_VDEC] +
                                                  vcu->gce_th_num[VCU_VENC]);
 ...
 dev_dbg(dev,
         "[VCU] GCE clt_base %p clt_vdec %d %p %p clt_venc %d %p %p %p dev %p",
         vcu->clt_base, vcu->gce_th_num[VCU_VDEC], vcu->clt_vdec[0],
         vcu->clt_vdec[1], vcu->gce_th_num[VCU_VENC], vcu->clt_venc[0],
         vcu->clt_venc[1], vcu->clt_venc_sec[0], dev);

 for (i = 0; i < GCE_EVENT_MAX; i++)
  vcu->gce_codec_eid[i] = -1;
 vcu->gce_codec_eid[VDEC_EVENT_0] = cmdq_dev_get_event(dev, "vdec_pic_start");
 vcu->gce_codec_eid[VDEC_EVENT_1] = cmdq_dev_get_event(dev, "vdec_decode_done");
 vcu->gce_codec_eid[VDEC_EVENT_2] = cmdq_dev_get_event(dev, "vdec_pause");
 vcu->gce_codec_eid[VDEC_EVENT_3] = cmdq_dev_get_event(dev, "vdec_dec_error");
 ...
 vcu->gce_codec_eid[VENC_MB_DONE] = cmdq_dev_get_event(dev, "venc_mb_done");
 vcu->gce_codec_eid[VENC_128BYTE_CNT_DONE] =
     cmdq_dev_get_event(dev, "venc_128B_cnt_done");
 vcu->gce_codec_eid[VENC_EOF_C1] = cmdq_dev_get_event(dev, "venc_eof_c1");
 ...
 vcu->gce_codec_eid[VENC_SPS_DONE] = cmdq_dev_get_event(dev, "venc_sps_done");
 vcu->gce_codec_eid[VENC_PPS_DONE] = cmdq_dev_get_event(dev, "venc_pps_done");

 for (i = 0; i < (int)VCU_CODEC_MAX; i++) {
  vcu->gce_cmds[i] = devm_kzalloc(dev, sizeof(struct gce_cmds), GFP_KERNEL);
...
void mtk_vcu_mem_release(struct mtk_vcu_queue *vcu_queue) {
 ...
 list_for_each_safe(p, q, &vcu_queue->pa_pages.list) {
  tmp = list_entry(p, struct vcu_pa_pages, list);
  cmdq_mbox_buf_free(vcu_queue->cmdq_dev, (void *)(unsigned long)tmp->kva,
                     (dma_addr_t)tmp->pa);
...
void *mtk_vcu_set_buffer(struct mtk_vcu_queue *vcu_queue,
 ...
 pr_debug("[%s] %d iova = %llx src_vb = %p dst_vb = %p\n", __func__,
          vcu_queue->num_buffers, mem_buff_data->iova, src_vb, dst_vb);
 ...
 if (dbuf == NULL && src_vb != NULL)
  for (plane = 0; plane < src_vb->num_planes; plane++) {
   dma_addr =
       src_vb->vb2_queue->mem_ops->cookie(src_vb->planes[plane].mem_priv);
   if (*dma_addr == mem_buff_data->iova) {
    ...
    pr_debug("src size = %d mem_buff_data len = %d\n",
             (unsigned int)vcu_buffer->size, (unsigned int)mem_buff_data->len);
   }
  }
 if (dbuf == NULL && dst_vb != NULL)
  for (plane = 0; plane < dst_vb->num_planes; plane++) {
   dma_addr =
       dst_vb->vb2_queue->mem_ops->cookie(dst_vb->planes[plane].mem_priv);
   if (*dma_addr == mem_buff_data->iova) {
 ...
 pr_debug(
     "[%s] Num_buffers = %d iova = %llx dbuf = %p size = %d mem_priv = %lx\n",
     __func__, vcu_queue->num_buffers, mem_buff_data->iova, vcu_buffer->dbuf,
     (unsigned int)vcu_buffer->size, (unsigned long)vcu_buffer->mem_priv);
...
void *mtk_vcu_get_buffer(struct mtk_vcu_queue *vcu_queue,
 ...
 vcu_buffer->mem_priv =
     vcu_queue->mem_ops->alloc(vcu_queue->dev, 0, mem_buff_data->len, 0, 0);
 ...
 pr_debug(
     "[%s] Num_buffers = %d iova = %llx va = %llx size = %d mem_priv = %lx\n",
     __func__, vcu_queue->num_buffers, mem_buff_data->iova, mem_buff_data->va,
     (unsigned int)vcu_buffer->size, (unsigned long)vcu_buffer->mem_priv);
...
void *mtk_vcu_get_page(struct mtk_vcu_queue *vcu_queue,
 ...
 mem_priv = cmdq_mbox_buf_alloc(vcu_queue->cmdq_dev, &temp_pa);
  ...
  for (buffer = 0; buffer < num_buffers; buffer++) {
   ...
   if (mem_buff_data->va == CODEC_MSK((unsigned long)cook) &&
    ...
    pr_debug("Free buff = %d iova = %llx va = %llx, queue_num = %d\n", buffer,
             mem_buff_data->iova, mem_buff_data->va, num_buffers);
    vcu_queue->mem_ops->put(vcu_buffer->mem_priv);
...
void mtk_vcu_buffer_ref_dec(struct mtk_vcu_queue *vcu_queue, void *mem_priv) {
 struct mtk_vcu_mem *vcu_buffer;
...
void vcu_io_buffer_cache_sync(struct device *dev, struct dma_buf *dbuf,
                              int op) {
...
int vcu_buffer_flush_all(struct device *dev, struct mtk_vcu_queue *vcu_queue) {
 ...
 for (buffer = 0; buffer < num_buffers; buffer++) {
  ...
  pr_debug("Cache clean %s buffer=%d iova=%lx size=%d num=%d\n",
           (vcu_buffer->dbuf == NULL) ? "working" : "io", buffer,
           (unsigned int long)vcu_buffer->iova, (unsigned int)vcu_buffer->size,
           num_buffers);

  if (vcu_buffer->dbuf == NULL) {
   cook = vcu_queue->mem_ops->vaddr(vcu_buffer->mem_priv);
   dmac_map_area((void *)cook, vcu_buffer->size, DMA_TO_DEVICE);
  } else
   vcu_io_buffer_cache_sync(dev, vcu_buffer->dbuf, DMA_TO_DEVICE);
 }
 ...
 for (buffer = 0; buffer < num_buffers; buffer++) {
  ...
  if ((dma_addr + size) <= (vcu_buffer->iova + vcu_buffer->size) &&
   ...
   pr_debug("Cache %s %s buffer iova=%lx range=%d (%lx %d)\n",
            ...
            (unsigned long)dma_addr, (unsigned int)size,
            (unsigned long)vcu_buffer->iova, (unsigned int)vcu_buffer->size);
